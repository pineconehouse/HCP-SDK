...
import com.hitachivantara.core.http.conn.HttpConnectionBase;
...
public class HttpURLConnection extends HttpConnectionBase {
	private java.net.HttpURLConnection connection;
	private Header[] headers;

	public HttpURLConnection(ClientConfiguration config) {
		super(config);
	}

	@Override
	public HttpResponse request(HttpRequest request) throws HttpException {
		try {
...
//			}
			...
			if (httpContent != null) {
				...
				// int retry = 1;//config.getMaxErrorRetry();
			...
			HttpResponse response = new DefaultHttpResponse(this);
	...
	public Header[] getAllHeaders() {
		if (this.headers == null) {
			Map<String, List<String>> conheaders = connection.getHeaderFields();
			headers = new Header[conheaders.size()];
			int i = 0;
			for (Iterator<String> it = conheaders.keySet().iterator(); it.hasNext();) {
				String name = it.next();
				String value = null;
				List<String> listvalue = conheaders.get(name);
				if (listvalue != null && listvalue.size() > 0) {
					value = listvalue.get(0);
				}
